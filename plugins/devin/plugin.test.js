...
const WINDOWS_CREDENTIALS_PATH = "~/AppData/Local/devin/credentials.toml"
const LINUX_STATE_DB = "~/.config/Devin/User/globalStorage/state.vscdb"
const MAC_STATE_DB = "~/Library/Application Support/Devin/User/globalStorage/state.vscdb"
...
function mockAppAuth(ctx, apiKey = "devin-session-token$app", stateDb = MAC_STATE_DB) {
  ctx.host.fs.writeText(stateDb, "db")
  ctx.host.sqlite.query.mockImplementation((db, sql) => {
    expect(String(sql)).toContain("windsurfAuthStatus")
    return db === stateDb ? makeAuthStatus(apiKey) : "[]"
  })
}
...
describe("devin plugin", () => {
  ...
  it("loads Windows CLI credentials from AppData Local", async () => {
    const ctx = makeCtx()
    ctx.host.fs.writeText(WINDOWS_CREDENTIALS_PATH, makeCredentialsToml())
    ctx.host.http.request.mockReturnValue({
      status: 200,
      bodyText: JSON.stringify(makeQuotaResponse()),
    })
  ...
  it("falls back to Linux Devin app SQLite auth via app-support discovery", async () => {
    const ctx = makeCtx()
    ctx.host.fs.writeText(LINUX_STATE_DB, "db")
    mockAppAuth(ctx, "devin-session-token$linux", LINUX_STATE_DB)
    ctx.host.http.request.mockReturnValue({
      status: 200,
    ...
    expect(ctx.host.sqlite.query).toHaveBeenCalledWith(
      LINUX_STATE_DB,
      expect.stringContaining("windsurfAuthStatus")
    )
  })
  ...
  it("reads auth from the Devin - Next app when stable Devin is absent", async () => {
    const ctx = makeCtx()
    ctx.host.fs.writeText(NEXT_STATE_DB, "db")
    ctx.host.sqlite.query.mockImplementation((db, sql) => {
      expect(String(sql)).toContain("windsurfAuthStatus")
    ...
    expect(queriedDbs).toEqual([NEXT_STATE_DB])
  ...
  it("falls back from a stale stable-Devin token to the Devin - Next app", async () => {
    const ctx = makeCtx()
    ctx.host.fs.writeText(MAC_STATE_DB, "db")
    ctx.host.fs.writeText(NEXT_STATE_DB, "db")
    ctx.host.sqlite.query.mockImplementation((db, sql) => {
      expect(String(sql)).toContain("windsurfAuthStatus")
      if (db === MAC_STATE_DB) return makeAuthStatus("devin-session-token$stable")
      if (db === NEXT_STATE_DB) return makeAuthStatus("devin-session-token$next")
      return "[]"
    })
