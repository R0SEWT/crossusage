...
|---|---|---|
| 1 | Devin CLI | See platform paths below (`credentials.toml`) |
| 2 | Devin app | `state.vscdb` / `windsurfAuthStatus` (Devin app dir, legacy Windsurf fallback) |

### CLI credentials (`devin auth login`)

| OS | Path |
|---|---|
| Linux / macOS | `~/.local/share/devin/credentials.toml` |
| Windows | `%LOCALAPPDATA%\devin\credentials.toml` |
| Legacy (migration) | `~/.local/share/cognition/credentials.toml` |

### Devin Desktop app (`state.vscdb`)

Per [Devin Desktop FAQ](https://docs.devin.ai/desktop/devin-desktop-faq). CrossUsage also reads legacy Windsurf paths during the transition.

| OS | Devin path | Legacy read fallback |
|---|---|---|
| macOS | `~/Library/Application Support/Devin/User/globalStorage/state.vscdb` | `~/Library/Application Support/Windsurf/...` |
| Linux | `~/.config/Devin/User/globalStorage/state.vscdb` | `~/.config/Windsurf/...` |
| Windows | `%APPDATA%\Devin\User\globalStorage\state.vscdb` | `%APPDATA%\Windsurf\...` |
...
```bash
sqlite3 ~/.config/Devin/User/globalStorage/state.vscdb \
  "SELECT value FROM ItemTable WHERE key = 'windsurfAuthStatus'"
```
...
```powershell
sqlite3 "$env:APPDATA\Devin\User\globalStorage\state.vscdb" "SELECT value FROM ItemTable WHERE key = 'windsurfAuthStatus' LIMIT 1;"
```
...
| `userStatus.planStatus.dailyQuotaRemainingPercent` | Daily quota percent when visible; weekly fallback only when weekly percent **and** weekly reset are both missing and daily quota is hidden |
| `userStatus.planStatus.weeklyQuotaRemainingPercent` | Weekly quota percent remaining. Proto3 JSON omits zeros — a weekly reset with no percentage is treated as **0 remaining / 100% used** |
| `userStatus.planStatus.dailyQuotaResetAtUnix` | Daily reset time when daily quota is visible |
| `userStatus.planStatus.weeklyQuotaResetAtUnix` | Weekly reset time; also identifies an exhausted weekly window when the remaining percent is omitted |
...
A present-but-unparsable weekly percentage is quota-unavailable (schema drift), not an exhausted window. Despite remaining-percent field names, tiles show percent **used** (`100 - remaining`).
